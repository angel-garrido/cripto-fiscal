...
def calcular_fifo(ventas_df, entradas_df):
    ...
    for _, e in entradas_df.iterrows():
        inventario.append({'fecha': e['fecha'], 'moneda': e['moneda'], 'cantidad': float(e['cantidad']),
    ...
    for _, venta in ventas_df.iterrows():
        ...
        while rest > 0 and inventario:
            ...
            ent['cantidad'] -= usado
            if ent['cantidad'] <= 1e-8: inventario.remove(ent)
            rest -= usado
...
resumen_anual = pd.DataFrame({
    'Año': range(2020, 2026),
    'Ganancia_Patrimonial': 0.0,
    'Rendimientos_Recompensas': 0.0,
    'Rendimientos_Minería': 0.0,
...
resumen_anual = resumen_anual.fillna(0).reset_index()

# Agrupado por Cripto (la que querías)
...
with pd.ExcelWriter(archivo_salida, engine='openpyxl') as writer:
    ...
    pd.DataFrame({
        'Concepto': ["Ganancia Patrimonial", "Rendimientos Recompensas", "Rendimientos Minería", "Referral Commission"],
        'Dónde declararlo': ["1800-1814 (F2)", "0033", "0033", "0304 (Base General)"],
        'Notas': ["Pestaña Agrupado por Año", "Valor mercado", "Valor mercado", "Base General"]
    }).to_excel(writer, sheet_name="Instrucciones Renta España", index=False)

print("✅ Archivo generado correctamente:", archivo_salida)
print("   - Resumen Anual: Resumen por año")
print("   - Agrupado por Año: Tabla por cripto (ideal para declarar)")