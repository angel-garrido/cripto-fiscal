...
# Valor de adquisición correcto según normativa española
entradas['valor_total'] = entradas['total_eur'].fillna(0)

# Para recompensas y minería: usar valor de mercado en el momento de recepción
...
# Opción CONSERVADORA: todas las ventas excepto EUR
ventas = df[df['tipo'] == 'venta'].copy()
ventas_ganancia = ventas[~ventas['moneda'].isin(['EUR'])].copy()

print(f"Total ventas procesadas: {len(ventas_ganancia)}")
...
def calcular_fifo_espana(ventas_df, entradas_df):
    ...
    for _, venta in ventas_df.iterrows():
        ...
        total_transmision = float(venta['total_eur'])          # Valor de transmisión
        moneda = venta['moneda']
...
# Rendimientos del capital (Recompensas + Minería)
recompensas = df[df['tipo'] == 'recompensa']
mineria = df[df['tipo'] == 'minería']

recompensas_anual = recompensas.groupby('año')['valor_total'].sum().rename("Rendimientos_Recompensas")
mineria_anual = mineria.groupby('año')['valor_total'].sum().rename("Rendimientos_Minería")

ganancias_patrimoniales = resumen_fifo.groupby('Año')['beneficio'].sum().rename("Ganancia_Patrimonial_Ventas")

resumen_anual = pd.concat([ganancias_patrimoniales, recompensas_anual, mineria_anual], axis=1).fillna(0).reset_index()
resumen_anual = resumen_anual.rename(columns={'Año': 'Año'})
...
with pd.ExcelWriter(archivo_salida, engine='openpyxl') as writer:
    ...
    # Hoja de instrucciones para la declaración
    instrucciones = pd.DataFrame({
        'Concepto': [
            "Ganancia/Pérdida Patrimonial (Ventas y permutas de cripto)",
            "Rendimientos del Capital Mobiliario - Recompensas (Earn, Staking, etc.)",
            "Rendimientos del Capital Mobiliario - Minería",
            "Total Base Imponible del Ahorro (aprox)"
        ],
        'Dónde declararlo en la Renta 2025': [
            "Casillas 1800 a 1814 (Ganancias y pérdidas patrimoniales)",
            "Casilla 0031 (Rendimientos del capital mobiliario)",
            "Casilla 0031 (Rendimientos del capital mobiliario)",
            "Suma de Ganancia Patrimonial + Rendimientos"
        ],
        'Observaciones': [
            "Se calcula con método FIFO. Incluye swaps (permutas cripto-cripto).",
            "Valor de mercado en el momento de recepción de la recompensa.",
            "Valor de mercado en el momento de recepción.",
            "Consulta con asesor fiscal. Conserva este Excel como justificante."
        ]
    })

    instrucciones.to_excel(writer, sheet_name="Instrucciones Renta España", index=False)

print(f"✅ Resumen fiscal generado correctamente: **{archivo_salida}**")
print("   → Revisa especialmente la hoja 'Instrucciones Renta España'")